"""
k と n を引数にとって，k × n のMDPC符号の生成行列をランダムに生成し，
符号語を列挙するスクリプト
LDPC符号のスクリプトで O(1) の部分を O(√n) に変えるだけ
"""
...
def LDPC_encode(k: str, n: str) -> None:
    ...
    ## MDPC符号のHamming重みは O(√n) で，ランダムに生成する
    ## [max(√n - log n, 1), min(√n + log n, n)] の範囲で考える
    exists = False
    while exists == False:
        range_lower = max(int(math.sqrt(n) - math.log2(n)), 1)
        range_higher = min(int(math.sqrt(n) + math.log2(n)), n)
        weight = random.randint(range_lower, range_higher)
        if(weight > 0):
